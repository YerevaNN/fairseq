...
path = "/home/gayane/BartLM/fairseq/scripts"

with open(f'{path}/YerevaNN Chemical Results - Apr25.csv') as f:
    r = csv.DictReader(f)
...
for task in tqdm(lines):
    ...
    if task['Type'] == 'Classification':
        ...
        print(p)
        for subtask in range(p, subtask_count):
            ...
            os.system(cmd)
            cmd = f"""CUDA_VISIBLE_DEVICES=0 fairseq-train {PATH1}{name}{PATH2} --update-freq {bs//2} --restore-file {BART_PATH} --wandb-project Fine_Tune_{name} --batch-size 2 --task multi_input_sentence_prediction --num-workers 1 --add-prev-output-tokens --layernorm-embedding --share-all-embeddings --share-decoder-input-output-embed --reset-optimizer --reset-dataloader --reset-meters --required-batch-size-multiple 1 --arch bart_large --skip-invalid-size-inputs-valid-test --criterion multi_input_sentence_prediction{noise_params} --max-target-positions 128 --max-source-positions 128 --dropout {drout} --optimizer adam --adam-betas "(0.9, 0.999)" --adam-eps 1e-08 --weight-decay 0.01 --attention-dropout 0.2 --relu-dropout 0.1 --clip-norm 0.1 --lr-scheduler polynomial_decay --lr {lr} --total-num-update {TOTAL_NUM_UPDATES} --max-update {TOTAL_NUM_UPDATES} --warmup-updates {WARMUP_UPDATES} --fp16 --keep-best-checkpoints 1 {keep_last_check} --num-classes {num_class} --save-dir {directory} >> /home/gayane/BartLM/Bart/chemical/log/{codename}.log"""
            print(cmd)
            os.system(cmd)
