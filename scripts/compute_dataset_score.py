...
for task in tqdm(lines):
    ...
    for subtask in range(subtask_count):
        ...
        cmd = f"python /home/gayane/BartLM/fairseq/scripts/compute_auc.py --lr {lr} --dataset-name {task_name} --subtask {str(int(subtask))} --warmup-update  {WARMUP_UPDATES} --total-number-update {TOTAL_NUM_UPDATES} " 
        print(cmd)
        os.system(cmd)
