...
path = "/home/gayane/BartLM/fairseq/examples/molbart/YerevaNN Chemical Results - Apr25.csv"
    ...
    for lr in learning_rate:
        for dropout in dropouts:
            if args.add_noise:
                for nt in noise:
                    for ld in lmb: 
                        row = f"{name},{regr_or_class},Yes,{dataset_size},{int(TOTAL_NUM_UPDATES*100)},{(subtask + 1) if args.single_task else minuts_1_task * subtask},{lr},{minuts_1_task},{Hours_to_train_all_subtasks},{dropout},{nt},{ld}"
                        f.write(row)
                        f.write('\n')
                        
            else:
                row = f"{name},{regr_or_class},Yes,{dataset_size},{int(TOTAL_NUM_UPDATES*100)},{(subtask + 1) if args.single_task else minuts_1_task * subtask},{lr},{minuts_1_task},{Hours_to_train_all_subtasks},{dropout}"
                f.write(row)
                f.write('\n')