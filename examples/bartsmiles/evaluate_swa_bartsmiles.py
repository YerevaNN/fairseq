...
with open('/home/gayane/BartLM/fairseq/examples/molbart/wandb_url.csv') as f:
    r = csv.DictReader(f)
...
for i in range(len(names)):
    ...
    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/compute_score.py --lr {lr} --dropout {drout}{noise_params} --dataset-name {task_name} --subtask 1 --warmup-update {warmup_updates} --total-number-update {total_num_update} --checkpoint_name checkpoint_best.pt >> /home/gayane/BartLM/Bart/chemical/log/{task_name}.log""" 
    print("---------------------> chkpt_name_best_val_loss: ", cmd)
    os.system(cmd)
    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/average_checkpoints.py--dataset-type test --inputs {directory}/ --output {directory}/{chkpt_name_best_val_loss}.pt --checkpoint-upper-bound {upper_bound_best_val_loss} --num-epoch-checkpoints {chkpt_count}""" 
    print("---------------------> chkpt_name_best_val_loss SWA: ")
    print(cmd)
    os.system(cmd)

    
    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/compute_score.py --lr {lr} --dropout {drout} {noise_params} --dataset-name {task_name} --subtask 1 --warmup-update {warmup_updates} --total-number-update {total_num_update} --checkpoint_name {chkpt_name_best_val_loss}.pt >> /home/gayane/BartLM/Bart/chemical/log/{task_name}.log""" 
    ...
    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/average_checkpoints.py --inputs {directory}/ --output {directory}/{chkpt_name_best_val_acc}.pt --checkpoint-upper-bound {upper_bound_best_val_acc} --num-epoch-checkpoints {chkpt_count}""" 
    print("---------------------> chkpt_name_best_val_acc SWA: ", cmd)
    os.system(cmd)


    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/compute_score.py --lr {lr} --dropout {drout} {noise_params} --dataset-name {task_name} --subtask 1 --warmup-update {warmup_updates} --total-number-update {total_num_update} --checkpoint_name {chkpt_name_best_val_acc}.pt >> /home/gayane/BartLM/Bart/chemical/log/{task_name}.log""" 
    print("---------------------> chkpt_name_best_val_acc SWA score: ", cmd)
    os.system(cmd)


    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/compute_score.py --lr {lr} --dropout {drout} --dataset-name {task_name} {noise_params} --subtask 1 --warmup-update {warmup_updates} --total-number-update {total_num_update} --checkpoint_name checkpoint{best_val_accuracy[i]}.pt >> /home/gayane/BartLM/Bart/chemical/log/{task_name}.log""" 
    print("--------------------->  chkpt_name_best_val_acc score: ")
    print(cmd)
    os.system(cmd)

    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/average_checkpoints.py --inputs {directory}/ --output {directory}/{chkpt_name_last}.pt --checkpoint-upper-bound 11 --num-epoch-checkpoints {chkpt_count}""" 
    print(" --------------------> last checkpoints SWA: ", cmd)
    os.system(cmd)


    cmd = f"""python /home/gayane/BartLM/fairseq/examples/molbart/compute_score.py --lr {lr} --dropout {drout} --dataset-name {task_name} {noise_params} --subtask 1 --warmup-update {warmup_updates} --total-number-update {total_num_update} --checkpoint_name {chkpt_name_last}.pt >> /home/gayane/BartLM/Bart/chemical/log/{task_name}.log""" 
